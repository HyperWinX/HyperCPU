...
TEST(INSTRUCTIONS, ADDWITHCARRY_b16_RM_R){
    ...
    cpu._memory[0x0104] = HLT;
    cpu._memory[0x010A] = 5;
    *reinterpret_cast<uint16_t*>(&cpu._xRegs[0]) = 0x0A0A;
    cpu._xRegs[1] = 0x010A;
    cpu._carry = true;
    EXPECT_EQ(cpu.Execute(), EXIT_HALT);
...
TEST(INSTRUCTIONS, ADDWITHCARRY_b8_R_RM){
    ...
    cpu._memory[0x0104] = HLT;
    cpu._memory[0x010A] = 5;
    *reinterpret_cast<uint8_t*>(&cpu._xRegs[1] + 3) = 0x0A;
    cpu._xRegs[0] = 0x010A;
    cpu._carry = true;
    EXPECT_EQ(cpu.Execute(), EXIT_HALT);
...
TEST(INSTRUCTIONS, ADDWITHCARRY_b16_R_RM){
    ...
    cpu._memory[0x0104] = HLT;
    cpu._memory[0x010A] = 5;
    *reinterpret_cast<uint16_t*>(&cpu._xRegs[1]) = 0x0A0A;
    cpu._xRegs[0] = 0x010A;
    cpu._carry = true;
    EXPECT_EQ(cpu.Execute(), EXIT_HALT);
    EXPECT_EQ(*reinterpret_cast<uint16_t*>(&cpu._memory[0x010A]), 5 + 0x0A0A + 1);
...
TEST(INSTRUCTIONS, ADDWITHCARRY_b16_R_IMM){
    ...
    cpu._memory[0x0104] = 0x0A;
    cpu._memory[0x0107] = HLT;
    *reinterpret_cast<uint16_t*>(&cpu._xRegs[1]) = 5;
    cpu._carry = true;
    EXPECT_EQ(cpu.Execute(), EXIT_HALT);
    EXPECT_EQ(*reinterpret_cast<uint16_t*>(&cpu._xRegs[1]), 5 + 0x0A0A + 1);
    cpu.CleanUp();
}
