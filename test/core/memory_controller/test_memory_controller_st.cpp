...
TEST_F(mc_st_test, FETCH8) {
  ...
  for (std::size_t i = 0; i < 8; ++i)
    ASSERT_EQ(mcmt._fetch8(counter), BYTE);
}
...
TEST_F(mc_st_test, FETCH16) {
  ...
  for (std::size_t i = 0; i < 4; ++i)
    ASSERT_EQ(mcmt._fetch16(counter), WORD);
}
...
TEST_F(mc_st_test, FETCH32) {
  ...
  for (std::size_t i = 0; i < 2; ++i)
    ASSERT_EQ(mcmt._fetch32(counter), DWORD);
}

TEST_F(mc_st_test, FETCH64) {
  mcmt._load64(counter, QWORD);

  ASSERT_EQ(mcmt._fetch64(counter), QWORD);
}
...
TEST_F(mc_st_fail_test, FETCH8) {
  ASSERT_DEATH(mcmt._fetch8(counter), "");
}

TEST_F(mc_st_fail_test, FETCH16) {
  ASSERT_DEATH(mcmt._fetch16(counter), "");
}

TEST_F(mc_st_fail_test, FETCH32) {
  ASSERT_DEATH(mcmt._fetch32(counter), "");
}

TEST_F(mc_st_fail_test, FETCH64) {
  ASSERT_DEATH(mcmt._fetch64(counter), "");
}
...
TEST_F(mc_st_near_fail_test, FETCH8) {
  counter = 1023;
  ASSERT_EXIT({mcmt._fetch8(counter); exit(0); }, ::testing::ExitedWithCode(0), "");
  counter = 1024;
  ASSERT_EXIT({mcmt._fetch8(counter); exit(0); }, ::testing::KilledBySignal(SIGABRT), "");
}

TEST_F(mc_st_near_fail_test, FETCH16) {
  counter = 1022;
  ASSERT_EXIT({mcmt._fetch16(counter); exit(0); }, ::testing::ExitedWithCode(0), "");
  counter = 1023;
  ASSERT_EXIT({mcmt._fetch16(counter); exit(0); }, ::testing::KilledBySignal(SIGABRT), "");
}

TEST_F(mc_st_near_fail_test, FETCH32) {
  counter = 1020;
  ASSERT_EXIT({mcmt._fetch32(counter); exit(0); }, ::testing::ExitedWithCode(0), "");
  counter = 1021;
  ASSERT_EXIT({mcmt._fetch32(counter); exit(0); }, ::testing::KilledBySignal(SIGABRT), "");
}

TEST_F(mc_st_near_fail_test, FETCH64) {
  counter = 1016;
  ASSERT_EXIT({mcmt._fetch64(counter); exit(0); }, ::testing::ExitedWithCode(0), "");
  counter = 1017;
  ASSERT_EXIT({mcmt._fetch64(counter); exit(0); }, ::testing::KilledBySignal(SIGABRT), "");
}