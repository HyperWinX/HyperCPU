...
TEST_F(CPU_TESTING, MUL_b8_RM_R){
    cpu.memoryptr[0x0100] = MUL;
    ...
    *reinterpret_cast<uint8_t*>((char*)&cpu.xRegs[0] + 2) = 0x10;
    cpu.xRegs[1] = 0x010A;
    cpu.carry_flag = true;
    EXPECT_EQ(cpu.Execute(), EXIT_HALT);
    EXPECT_EQ(*reinterpret_cast<uint8_t*>(&cpu.memoryptr[0x010A]), 0x0A * 0x10);
}

TEST_F(CPU_TESTING, MUL_b16_RM_R){
    cpu.memoryptr[0x0100] = MUL;
    ...
    *reinterpret_cast<uint16_t*>((char*)&cpu.xRegs[0] + 2) = 0x0A0A;
    ...
    cpu.carry_flag = true;
    EXPECT_EQ(cpu.Execute(), EXIT_HALT);
    EXPECT_EQ(*reinterpret_cast<uint16_t*>(&cpu.memoryptr[0x010A]), static_cast<uint16_t>(0x2020 * 0x0A0A));
}

TEST_F(CPU_TESTING, MUL_b32_RM_R){
    cpu.memoryptr[0x0100] = MUL;
    ...
    cpu.memoryptr[0x0104] = HLT;
    *reinterpret_cast<uint32_t*>(&cpu.memoryptr[0x010A]) = 0x202020;
    cpu.xRegs[0] = 0x0A0A0A0A;
    cpu.xRegs[1] = 0x010A;
    cpu.carry_flag = true;
    EXPECT_EQ(cpu.Execute(), EXIT_HALT);
    EXPECT_EQ(*reinterpret_cast<uint32_t*>(&cpu.memoryptr[0x010A]),
                static_cast<uint32_t>(0x202020) * static_cast<uint32_t>(0x0A0A0A0A));
}

TEST_F(CPU_TESTING, MUL_b8_R_RM){
    cpu.memoryptr[0x0100] = MUL;
...
TEST_F(CPU_TESTING, MUL_b16_R_RM){
    cpu.memoryptr[0x0100] = MUL;
...
TEST_F(CPU_TESTING, MUL_b32_R_RM){
    cpu.memoryptr[0x0100] = MUL;
...
TEST_F(CPU_TESTING, MUL_b8_RM_IMM){
    cpu.memoryptr[0x0100] = MUL;
    ...
    cpu.memoryptr[0x010A] = 0x10;
    cpu.xRegs[1] = 0x010A;
    cpu.carry_flag = true;
    EXPECT_EQ(cpu.Execute(), EXIT_HALT);
    EXPECT_EQ(*reinterpret_cast<uint8_t*>(&cpu.memoryptr[0x010A]), 0x10 * 0x0A);
}

TEST_F(CPU_TESTING, MUL_b16_RM_IMM){
    cpu.memoryptr[0x0100] = MUL;
    ...
    cpu.memoryptr[0x0107] = HLT;
    *reinterpret_cast<uint16_t*>(&cpu.memoryptr[0x010A]) = 0x220;
    cpu.xRegs[1] = 0x010A;
    cpu.carry_flag = true;
    EXPECT_EQ(cpu.Execute(), EXIT_HALT);
    EXPECT_EQ(*reinterpret_cast<uint16_t*>(&cpu.memoryptr[0x010A]), static_cast<uint16_t>(0x220 * 0x0A0A));
}

TEST_F(CPU_TESTING, MUL_b32_RM_IMM){
    cpu.memoryptr[0x0100] = MUL;
...
TEST_F(CPU_TESTING, MUL_b8_R_IMM){
    cpu.memoryptr[0x0100] = MUL;
    cpu.memoryptr[0x0101] = (static_cast<uint8_t>(HyperCPU::b8) << 4) | static_cast<uint8_t>(HyperCPU::R_IMM);
    cpu.memoryptr[0x0102] = HyperCPU::xlh1;
    cpu.memoryptr[0x0103] = 0x0A;
    cpu.memoryptr[0x0107] = HLT;
    *reinterpret_cast<uint8_t*>((char*)&cpu.xRegs[1] + 2) = 0x10;
    cpu.carry_flag = true;
    EXPECT_EQ(cpu.Execute(), EXIT_HALT);
    EXPECT_EQ(*reinterpret_cast<uint8_t*>((char*)&cpu.xRegs[1] + 2), 0x10 * 0x0A);
}

TEST_F(CPU_TESTING, MUL_b16_R_IMM){
    cpu.memoryptr[0x0100] = MUL;
...
TEST_F(CPU_TESTING, MUL_b32_R_IMM){
    cpu.memoryptr[0x0100] = MUL;
...
TEST_F(CPU_TESTING, MUL_b8_RM_M){
    cpu.memoryptr[0x0100] = MUL;
...
TEST_F(CPU_TESTING, MUL_b16_RM_M){
    cpu.memoryptr[0x0100] = MUL;
...
TEST_F(CPU_TESTING, MUL_b32_RM_M){
    cpu.memoryptr[0x0100] = MUL;
...
TEST_F(CPU_TESTING, MUL_b16_R_M){
    cpu.memoryptr[0x0100] = MUL;
...
TEST_F(CPU_TESTING, MUL_b32_R_M){
    cpu.memoryptr[0x0100] = MUL;
...
TEST_F(CPU_TESTING, MUL_b16_M_R){
    cpu.memoryptr[0x0100] = MUL;
...
TEST_F(CPU_TESTING, MUL_b32_M_R){
    cpu.memoryptr[0x0100] = MUL;
...
TEST_F(CPU_TESTING, MUL_b8_M_RM){
    cpu.memoryptr[0x0100] = MUL;
...
TEST_F(CPU_TESTING, MUL_b16_M_RM){
    cpu.memoryptr[0x0100] = MUL;
...
TEST_F(CPU_TESTING, MUL_b32_M_RM){
    cpu.memoryptr[0x0100] = MUL;
