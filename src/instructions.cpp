...
std::tuple<int, std::function<void(void*)> HyperCPU::CPU::_get_sa(HyperCPU::datasize_t size, HyperCPU::argtp_t args){
    int size = 0;
    std::function<void(void*)> access_func;
    switch(size){
        case b8:        size = 1;
        case b16:       size = 2;
        case b32:       size = 4;
        case bUNKNOWN:  size = 0;
    }
    switch(args){
...
int HyperCPU::CPU::_ins_adc_exec(HyperCPU::_instruction_t &instr, void* ptr1, void* ptr2) {
    ...
    switch (instr.args){
        case RM_R:
            std::tuple<int, int> size_action = _get_sa(instr.size, instr.args);
            uint32_t data = 
            break;
        case R_RM:
            if (instr.size == b8)
