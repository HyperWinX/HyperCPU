...
struct Operand {
  ...
  enum Mode mode;
  bool needs_resolve;
  std::array<hpool::Ptr<pog::TokenWithLineSpec<Value>,
                        hpool::ReallocationPolicy::OffsetRealloc>,
             2>
      tokens;
  std::variant<std::uint64_t, std::int64_t, std::shared_ptr<std::string>>
      variant;
};
...
struct Value {
  std::variant<std::int64_t, std::uint64_t, std::string, Operand, Instruction>
      val;
};
...
struct BinaryResult {
  ...
  template <UnsignedIntegral T> constexpr inline void push(T data) {
...
template <typename... T> struct MakeOverload : T... {
  using T::operator()...;
};

template <typename... T> MakeOverload(T...) -> MakeOverload<T...>;

template <typename Variant, typename... Alternatives>
constexpr inline decltype(auto) VisitVariant(Variant &&variant,
                                             Alternatives &&...alternatives) {
  return std::visit(MakeOverload{std::forward<Alternatives>(alternatives)...,
                                 [](auto const &) {}},
                    variant);
}
...
struct CompilerState {
  CompilerState(hpool::HPool<pog::TokenWithLineSpec<Value>,
                             hpool::ReallocationPolicy::OffsetRealloc> &pool)
  ...
  hpool::HPool<pog::TokenWithLineSpec<Value>,
               hpool::ReallocationPolicy::OffsetRealloc> &pool;
...
std::string_view FindLine(const pog::LineSpecialization &,
                          const std::string_view &);
void WriteResultFile(HyperCPU::FileType type, HCAsm::BinaryResult &result,
                     std::ofstream &output, std::uint32_t code_size,
                     std::uint32_t entry_point);

[[noreturn]] void ThrowError(pog::TokenWithLineSpec<Value> &err_token,
                             pog::Parser<Value> &parser, std::string err_msg);
...
Value ParseOperand1(pog::Parser<Value> &,
                    std::vector<pog::TokenWithLineSpec<Value>> &&args);
Value ParseOperand2(pog::Parser<Value> &,
                    std::vector<pog::TokenWithLineSpec<Value>> &&args);
Value ParseOperand3(pog::Parser<Value> &,
                    std::vector<pog::TokenWithLineSpec<Value>> &&args);
Value ParseOperand4(pog::Parser<Value> &,
                    std::vector<pog::TokenWithLineSpec<Value>> &&args);
Value ParseOperand5(pog::Parser<Value> &,
                    std::vector<pog::TokenWithLineSpec<Value>> &&args);
Value ParseOperand6(pog::Parser<Value> &,
                    std::vector<pog::TokenWithLineSpec<Value>> &&args);
Value ParseOperand7(pog::Parser<Value> &,
                    std::vector<pog::TokenWithLineSpec<Value>> &&args);
Value ParseOperand8(pog::Parser<Value> &,
                    std::vector<pog::TokenWithLineSpec<Value>> &&args);
Value ParseOperand9(pog::Parser<Value> &,
                    std::vector<pog::TokenWithLineSpec<Value>> &&args);
Value ParseOperand10(pog::Parser<Value> &,
                     std::vector<pog::TokenWithLineSpec<Value>> &&args);
Value ParseOperand11(pog::Parser<Value> &,
                     std::vector<pog::TokenWithLineSpec<Value>> &&args);
Value ParseOperand12(pog::Parser<Value> &,
                     std::vector<pog::TokenWithLineSpec<Value>> &&args);

Value CompileStatement1(pog::Parser<Value> &,
                        std::vector<pog::TokenWithLineSpec<Value>> &&args);
Value CompileStatement2(pog::Parser<Value> &,
                        std::vector<pog::TokenWithLineSpec<Value>> &&args);
Value CompileStatement3(pog::Parser<Value> &,
                        std::vector<pog::TokenWithLineSpec<Value>> &&args);
Value CompileEntryLabel(pog::Parser<Value> &,
                        std::vector<pog::TokenWithLineSpec<Value>> &&args);
Value CompileLabel(pog::Parser<Value> &,
                   std::vector<pog::TokenWithLineSpec<Value>> &&args);
Value CompileRawValueb8_str(pog::Parser<Value> &,
                            std::vector<pog::TokenWithLineSpec<Value>> &&args);
Value CompileRawValueb8(pog::Parser<Value> &,
                        std::vector<pog::TokenWithLineSpec<Value>> &&args);
Value CompileRawValueb16(pog::Parser<Value> &,
                         std::vector<pog::TokenWithLineSpec<Value>> &&args);
Value CompileRawValueb32(pog::Parser<Value> &,
                         std::vector<pog::TokenWithLineSpec<Value>> &&args);
Value CompileRawValueb64(pog::Parser<Value> &,
                         std::vector<pog::TokenWithLineSpec<Value>> &&args);
