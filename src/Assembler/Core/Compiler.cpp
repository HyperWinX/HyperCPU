...
HCAsm::HCAsmCompiler::HCAsmCompiler(LogLevel lvl) : pool(32) {
  ...
  parser.token(":").symbol(":");
  parser.token(R"(\n)").action(
      [this]([[maybe_unused]] std::string_view tok) -> Value {
        ++parser.get_line_counter();
        parser.reset_line_offset();
        return {};
      });

  parser.token(R"(\.attr\(entry\))").symbol("entry");

  parser.token("#use").fullword().symbol("use");
  parser.token("\\.b8").fullword().symbol(".b8");
  parser.token("\\.b16").fullword().symbol(".b16");
  parser.token("\\.b32").fullword().symbol(".b32");
  parser.token("\\.b64").fullword().symbol(".b64");
  ...
  parser.token(R"(0s[0-9]+)").symbol("sint").action(TokenizeSignedInt);
  parser.token(R"(0u[0-9]+)").symbol("uint").action(TokenizeUnsignedInt);
  parser.token(R"(0x[0-9a-fA-F]+)").symbol("hex").action(TokenizeHexadecimal);
  parser.token(R"(0b[0-1]+)").symbol("binary").action(TokenizeBinary);
  parser.token(R"('.')").symbol("char").action(TokenizeChar);
  ...
  parser.rule("operand")
      ...
      .production("ident", "ident", "[", "ident", "+", "uint", "]",
                  ParseOperand6)
      .production("ident", "ident", "[", "ident", "+", "hex", "]",
                  ParseOperand6)
      .production("ident", "ident", "[", "ident", "+", "binary", "]",
                  ParseOperand6)
...
HCAsm::BinaryResult HCAsm::HCAsmCompiler::Compile(std::string &contents,
...
HCAsm::HCAsmCompiler::OperandSize(HCAsm::OperandType op) {
  switch (op) {
...
std::uint8_t HCAsm::HCAsmCompiler::InstructionSize(HCAsm::Instruction &instr) {
  ...
  std::uint8_t result =
      3; // Opcode is always two bytes long + one byte for operand types
...
HCAsm::BinaryResult
HCAsm::HCAsmCompiler::TransformToBinary(HCAsm::CompilerState &ir) {
    ...
    VisitVariant(
        ...
        [&ir](RawValue &raw) mutable -> void {
          ir.code_size += [&raw]() -> std::uint8_t {
            switch (raw.mode) {
            case Mode::b8_str:
              return std::get<std::shared_ptr<std::string>>(raw.value.variant)
                  ->size();
            case Mode::b8:
              return 1;
            case Mode::b16:
  ...
  logger.Log(LogLevel::DEBUG,
             fmt::format("{} label references are waiting for resolve",
                         ir.pending_resolves.size()));
  if (!ir.pending_resolves.empty()) {
    ...
    for (auto &args : ir.pending_resolves) {
      ...
      auto *op = args.op ? &std::get<Instruction>(instr).op2
                         : &std::get<Instruction>(instr).op1;

      if (ir.labels.contains(
              *std::get<std::shared_ptr<std::string>>(op->variant))) {
        op->type = OperandType::uint;
        op->variant.emplace<0>(
            ir.labels[*std::get<std::shared_ptr<std::string>>(op->variant)]);
      } else {
        ThrowError(
            *op->tokens[0], parser,
            fmt::format("failed to resolve undefined reference to \"{}\"",
                        *std::get<std::shared_ptr<std::string>>(op->variant)));
      }
    }
  }
  ...
  for (auto &instr : ir.ir) {
    VisitVariant(
        ...
        [&binary, &ir, this](RawValue &raw) mutable -> void {
          ...
          case Mode::b8_str:
            binary.push(
                *std::get<std::shared_ptr<std::string>>(raw.value.variant));
            break;
          case Mode::b8:
            binary.push(static_cast<std::uint8_t>(
                std::get<std::uint64_t>(raw.value.variant)));
            break;
          case Mode::b16:
            binary.push(static_cast<std::uint16_t>(
                std::get<std::uint64_t>(raw.value.variant)));
            break;
          case Mode::b32:
            binary.push(static_cast<std::uint32_t>(
                std::get<std::uint64_t>(raw.value.variant)));
            break;
          case Mode::b64_label:
            if (!ir.labels.contains(*std::get<std::shared_ptr<std::string>>(
                    raw.value.variant))) {
              ThrowError(
                  *raw.value.tokens[1], parser,
                  fmt::format("failed to resolve undefined reference to \"{}\"",
                              *std::get<std::shared_ptr<std::string>>(
                                  raw.value.variant)));
            }
            binary.push(static_cast<std::uint64_t>(ir.labels.at(
                *std::get<std::shared_ptr<std::string>>(raw.value.variant))));
            break;
          case Mode::b64:
            binary.push(static_cast<std::uint64_t>(
                std::get<std::uint64_t>(raw.value.variant)));
            break;
          default:
            ABORT();
...
std::string_view HCAsm::FindLine(const pog::LineSpecialization &line_spec,
...
[[noreturn]] void HCAsm::ThrowError(pog::TokenWithLineSpec<Value> &err_token,
...
void HCAsm::WriteResultFile(HyperCPU::FileType type,
                            HCAsm::BinaryResult &result, std::ofstream &output,
                            std::uint32_t code_size,
