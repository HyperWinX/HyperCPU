...
namespace pog {
  ...
  public:
    ...
    Grammar()
        : _rules(), _symbols(), _name_to_symbol(), _internal_start_symbol(nullptr), _internal_end_of_input(nullptr),
    ...
    std::vector<const SymbolType*> get_terminal_symbols() const {
      ...
      transform_if(_symbols.begin(), _symbols.end(), std::back_inserter(result), [](const auto& s) { return s->is_terminal() || s->is_end(); }, [](const auto& s) { return s.get(); });
      return result;
    }

    std::vector<const SymbolType*> get_nonterminal_symbols() const {
      std::vector<const SymbolType*> result;
      transform_if(_symbols.begin(), _symbols.end(), std::back_inserter(result), [](const auto& s) { return s->is_nonterminal(); }, [](const auto& s) { return s.get(); });
      return result;
    }
    ...
    std::vector<const RuleType*> get_rules_of_symbol(const SymbolType* sym) const {
      ...
      transform_if(_rules.begin(), _rules.end(), std::back_inserter(result), [&](const auto& rule) { return sym == rule->get_lhs(); }, [](const auto& rule) { return rule.get(); });
      return result;
    }
    ...
    std::vector<const RuleType*> get_rules_with_symbol(const SymbolType* sym) const {
      std::vector<const RuleType*> result;
      transform_if(_rules.begin(), _rules.end(), std::back_inserter(result), [&](const auto& rule) { return std::find(rule->get_rhs().begin(), rule->get_rhs().end(), sym) != rule->get_rhs().end(); }, [](const auto& rule) { return rule.get(); });
      return result;
    }
