...
namespace pog {
  ...
  public:
    ...
    template <typename T>
    Error(T&& msg) noexcept
        : _msg(std::forward<T>(msg)) {
    }
  ...
  public:
    template <typename T>
    SyntaxError(const Symbol<T>* unexpected_symbol, const std::vector<const Symbol<T>*>& expected_symbols)
        : Error() {
    ...
    template <typename T>
    SyntaxError(const std::vector<const Symbol<T>*>& expected_symbols)
        : Error() {
      std::vector<std::string> expected_symbols_str(expected_symbols.size());
      std::transform(expected_symbols.begin(), expected_symbols.end(), expected_symbols_str.begin(), [](const auto& sym) {
        return sym->get_description();
