...
namespace pog {
  ...
  public:
    ...
    std::size_t number_of_issues() const {
      return _issues.size();
    }
    auto begin() {
    ...
    std::string to_string(std::string_view arrow = "->", std::string_view eps = "<eps>") const {
      ...
      std::transform(_issues.begin(), _issues.end(), issues_str.begin(), [&](const auto& issue) {
        return visit_with(issue, [&](const ShiftReduceConflictType& sr) { return sr.to_string(arrow, eps); }, [&](const ReduceReduceConflictType& rr) { return rr.to_string(arrow, eps); });
      });
