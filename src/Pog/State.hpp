...
namespace pog {
  ...
  public:
    ...
    std::vector<const ItemType*> get_production_items() const {
      ...
      transform_if(_items.begin(), _items.end(), std::back_inserter(result), [](const auto& item) { return item->is_final(); }, [](const auto& item) { return item.get(); });
      return result;
    }
    ...
    const std::map<const SymbolType*, const State*, SymbolLess<ValueT>>& get_transitions() const {
      return _transitions;
    }
    const std::map<const SymbolType*, std::vector<const State*>, SymbolLess<ValueT>>& get_back_transitions() const {
      return _back_transitions;
    }
