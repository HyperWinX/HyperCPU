...
namespace pog {
  ...
  public:
    ...
    const SymbolType* get_lhs() const {
      return _lhs;
    }
    const std::vector<const SymbolType*>& get_rhs() const {
      return _rhs;
    }

    bool has_precedence() const {
      return static_cast<bool>(_precedence);
    }
    const Precedence& get_precedence() const {
      return _precedence.value();
    }
    void set_precedence(std::uint32_t level, Associativity assoc) {
    ...
    bool is_start_rule() const {
      return _start;
    }

    void set_start_rule(bool set) {
      _start = set;
    }
    void set_midrule(std::size_t size) {
      _midrule_size = size;
    }
    bool is_midrule() const {
      return static_cast<bool>(_midrule_size);
    }
    std::size_t get_midrule_size() const {
      return _midrule_size.value();
    }
