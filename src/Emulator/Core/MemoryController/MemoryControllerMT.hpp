...
namespace HyperCPU {
  class MemoryControllerMT final : public IMemoryController {
  private:
    char* memory;
    class CPU* cpu;
  ...
  public:
    explicit MemoryControllerMT(std::size_t mem_size, class CPU* cpu = nullptr) : cpu(cpu), total_mem(mem_size) {
