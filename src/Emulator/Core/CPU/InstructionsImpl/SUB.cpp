...
void hypercpu::cpu::exec_sub(operand_types op_types, mode md, void* op1, void* op2) {
  switch (op_types) {
    case R_R: {
      switch (md) {
        case b8:
          udf = subtraction_will_underflow(deref<std::uint8_t>(op1), deref<std::uint8_t>(op2));
          deref<std::uint8_t>(op1) -= hypercpu::bit_cast_from<std::uint8_t>(op2);
          break;

        case b16:
          udf = subtraction_will_underflow(deref<std::uint16_t>(op1), deref<std::uint16_t>(op2));
          deref<std::uint16_t>(op1) -= hypercpu::bit_cast_from<std::uint16_t>(op2);
          break;

        case b32:
          udf = subtraction_will_underflow(deref<std::uint32_t>(op1), deref<std::uint32_t>(op2));
          deref<std::uint32_t>(op1) -= hypercpu::bit_cast_from<std::uint32_t>(op2);
          break;

        case b64:
          udf = subtraction_will_underflow(deref<std::uint64_t>(op1), deref<std::uint64_t>(op2));
    ...
    case R_RM: {
      ...
      switch (md) {
        case b8: {
          ...
          udf = subtraction_will_underflow(deref<std::uint8_t>(op1), val);
        ...
        case b16: {
          ...
          udf = subtraction_will_underflow(deref<std::uint16_t>(op1), val);
        ...
        case b32: {
          ...
          udf = subtraction_will_underflow(deref<std::uint32_t>(op1), val);
        ...
        case b64: {
          ...
          udf = subtraction_will_underflow(deref<std::uint64_t>(op1), val);
